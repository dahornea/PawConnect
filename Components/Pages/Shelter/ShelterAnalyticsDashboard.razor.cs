...
public partial class ShelterAnalyticsDashboard
{
    ...
    private ShelterAnalyticsDashboardDto? _dashboard;
    ...
    private async Task LoadAsync()
    {
        try
        {
            ...
            {
                _error = "You must be signed in as a shelter account to view analytics.";
                return;
            }

            _dashboard = await AnalyticsService.GetShelterAnalyticsAsync(BuildRange(), userId);
        }
        catch (Exception ex)
        {
            _error = ex.Message;
